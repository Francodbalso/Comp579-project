...
class ReplayBuffer():
    ...
    """
    def __init__(self, max_len, obs_dim):
        self.max_len = max_len
        #self.arr = np.zeros((max_len, 2*obs_dim+1+1+1))
        self.arr = np.zeros((max_len, 2*obs_dim+1+1))
        self.cur_ind = 0
        self.is_full = False
        self.obs_dim = obs_dim
    def add_entry(self, state, next_state, reward, done):
    #def add_entry(self, state, next_state, reward):
        ...
        self.arr[self.cur_ind, :]=np.concatenate((state, next_state, np.array([reward], dtype=float), np.array([done], dtype=float)))
